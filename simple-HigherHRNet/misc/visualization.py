...
def draw_points(image, points, color_palette='tab20', palette_samples=16, confidence_threshold=0.3):
 ...
 #       if int(pt[2]) > confidence_threshold:
 #           image = cv2.circle(image, (int(pt[1]), int(pt[0])), circle_size, tuple(colors[i % len(colors)]), -1)
    ...
    for i, pt in enumerate(points):
        ...
        for j in range(nof_j):
            ...
            y=int(y)
            #print(x,y)
            image = cv2.circle(image, (y, x), circle_size, tuple(colors[i % len(colors)]), -1)
...
"""
    for i, joint in enumerate(skeleton):
        ...
        joint1,joint2=joint
        pt1, pt2 = points[i,joint1],points[i,joint2]
